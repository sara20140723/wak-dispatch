...
@MapperScan(basePackages = "com.wak.dispatch.dao.dispatch", sqlSessionTemplateRef  = "db1SqlSessionTemplate")
public class DispatchOrderConfig {

    @Bean(name = "db1DataSource")
    // 读取 application.properties 中的配置参数映射成为一个对象, prefix表示参数的前缀
    @ConfigurationProperties(prefix = "wak.datasources.dispatch")
    // 表示这个数据源是默认数据源
    @Primary
    ...
    public SqlSessionFactory xwSqlSessionFactory(@Qualifier("db1DataSource") DataSource dataSource) throws Exception {
        ...
        bean.setMapperLocations(new PathMatchingResourcePatternResolver().getResources("classpath:mybatis/mapper/dispatch/*.xml"));
